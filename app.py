...
from jobs import model_evaluation_module
...
def logistic():
    ...
    if request.method == "POST":
        ...
        result, accuracy, precision, recall, f1 = (
            logistic_regression.handle_request(request.form)
        )
...
def classification_app():
    ...
    if request.method == "POST":
        ...
        result, accuracy, precision, recall, f1 = (
            classification_model.handle_request(request.form)
        )
...
def model_evaluation_view():

    evaluation_results = model_evaluation_module.get_results()
